...
const BlogIndexPage = () => (
  <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(56,217,199,0.14),_transparent_36%),linear-gradient(180deg,_#f5fbfa_0%,_#f0fdfa_100%)] text-slate-900">
      ...
      <div className="mt-12 grid gap-6">
        {blogPosts.length > 0 ? (
          blogPosts.map((post) => (
            <article
              key={post.slug}
              className="rounded-3xl border border-teal-100 bg-white/90 p-6 shadow-sm shadow-teal-100/60 transition-transform duration-200"
            >
              <div className="flex flex-wrap items-center gap-3 text-sm text-slate-500">
                ...
                {post.frontmatter.tags?.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-teal-50 px-3 py-1 text-teal-700"
                  >
                    {tag}
                  </span>
                ))}
          ...
          <section className="rounded-[2rem] border border-dashed border-teal-200 bg-white/80 p-8">
